 ...
 */
public class MyReportInfoCommentListViewAdapter extends BaseAdapter{
    private List<ReportInfo.CommentDataBean> data ;
    private Context mContext;
    private LayoutInflater layoutInflater;
    public MyReportInfoCommentListViewAdapter (List<ReportInfo.CommentDataBean> data,Context mContext){
        this.data = data;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       ReportInfo.CommentDataBean comment = data.get(position);
        hold.tv_comment.setText(comment.getContent());

        hold.tv_name.setText(comment.getUname());
        String time = new SimpleDateFormat(" HH:mm:ss").format(new Date(Long.parseLong(comment.getCtime()) * 1000));
        hold.tv_time.setText(time);
        Glide.with(mContext).load(comment.getUavatar()).into( hold.iv_icon);
        return convertView;
    }
